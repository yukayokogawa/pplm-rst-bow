...
def build_bows_one_hot_vectors(bow_indices, tokenizer, device='cuda'):
    ...
    for single_bow in bow_indices:
        ...
        #print(torch.sum(one_hot_bow.ne(0)))
...
class PPLMDecoding(BaseDecoding):
    ...
    def generate(self, model, classifier, proj, batch):
        ...
        #print(lm_labels)
        #print(self.tokenizer.convert_ids_to_tokens(lm_labels))
        disc_lens = batch['discourse_lengths']
        disc_lens = disc_lens[0]
        #print('disc_lens', disc_lens)
        edu_len_sum = disc_lens[0]-1
        edu_end_pos = [edu_len_sum]
        for length in disc_lens[1:]:
        ...
        for i,bow_edu in enumerate(bows):
            ...
            if bow_edu == []:
                ...
                """
                for word in bow_edu:
                    print(self.tokenizer.convert_ids_to_tokens(word))
                print('---')
                """
        ...
        while cur_len < max_tgt_len:
            perturb = False
            try:
                cur_edu_num = discourse_pos[cur_len]
            except:
                cur_edu_num = -1
            if cur_edu_num == 63:
                cur_edu_num = -1
            ...
            if cur_edu_num > 0:
                ...
                previous_edu_len = 0
            #cur_edu_len = disc_lens[cur_edu_num]
            if cur_edu_num > 0:
            ...
            if cur_edu_num >= 0:
                ...
                cur_label = discourse_labels[cur_len]
            ...
            if (not self.no_rst) and cur_edu_num >= 0 and (cur_label not in [self.mask_idx, 0]) and cur_bow != []:
                pert_past, pert_past_kv,  _, grad_norms, _ = perturb_past(
                    ...
                    loss_type=3,
                    num_iterations=3,
                    #horizon_length=(disc_lens[cur_edu_num]-cur_len),
                    horizon_length=1,
                    window_length=0,
                    decay=False,
                    gamma=1.5,
                ...
                perturb = True
            elif (not self.no_rst) and cur_edu_num >= 0 and cur_label not in [self.mask_idx, 0]:
                pert_past, pert_past_kv,  _, grad_norms, _ = perturb_past(
                    past,
                    model,
                    last,
                ...
                perturb = True
            elif cur_edu_num >= 0 and cur_bow != []:
                pert_past, pert_past_kv,  _, grad_norms, _ = perturb_past(
                    past,
                    model,
                    last,
            ...
            #print('pert_past_kv', pert_past_kv)
            #print('perturb',perturb)
            #print('encoder_outputs', encoder_outputs)
            #print('encoder_attn_mask', encoder_attn_mask)
            pert_outputs = model(
                decoder_input_ids=last,
            ...
            if cur_edu_num >= 0:
                pert_probs = ((pert_probs ** self.gm_scale) * (
                        unpert_probs ** (1 - self.gm_scale)))   + SMALL_CONST
            else:
                pert_probs = pert_probs
            pert_probs = top_k_filter(pert_probs, k=self.topk,
            ...
            #print(pert_probs)
            next_token = torch.multinomial(pert_probs, num_samples=1).squeeze(1)
            ...
            chosen_token_probs = pert_probs.gather(1, next_token.view(-1, 1))
