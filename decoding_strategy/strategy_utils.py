...
def make_past_pplm(past_key_values):
    past = []
    for layer, past_kv in enumerate(past_key_values):
        key, value = past_kv[:2]
        key = key.unsqueeze(0)
        value = value.unsqueeze(0)
        key_value = torch.cat((key, value), dim=0)
        past.append(key_value)
    return past

def make_past_kv_bart(past_key_values, past):
    past_kv = []
    #print('past_key_values', past_key_values)
    for layer, (p, p_kv) in enumerate(zip(past,past_key_values)):
        cross_key, cross_value = p_kv[-2:]
        key, value = torch.chunk(p, 2, dim=0)
        key = key.squeeze(0)
        value = value.squeeze(0)
        new_p = (key, value, cross_key, cross_value)
        past_kv.append(new_p)
    past_kv = tuple(past_kv)
    return past_kv

def get_past(past_key_values, b=0):
    past_kv = []
    for layer, past_kv in enumerate(past_key_values):
        key, value, cross_key, cross_value = p_kv
        key = key[b].unsqueeze(0)
        value = value[b].unsqueeze(0)
        cross_key = cross_key[b].unsqueeze(0)
        cross_value = cross_value[b].unsqueeze(0)
        new_p = (key, value, cross_key, cross_value)
        past_kv.append(new_p)
    past_kv = tuple(past_kv)
